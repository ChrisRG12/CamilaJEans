...
<div class="container">
    <div class="card text-center mb-5 mt-5 mx-auto" style="max-width: 400px; color:#F9F8F8">
      ...
      {{-- {{route('provedor.update', $consultaId->idProvedor)}} --}}
      <form method="POST" action="{{route('pedidos.update', $consultaId->id)}}">
        @csrf
      ...
      <div class="card-body">
        ...
        <div class="mb-3 d-flex">
          ...
          <div class="mb-3 d-flex">
            <label class="form-label me-3" style="color: black">Piezas:</label>
            <input type="number" class="form-control" name="txtpiezas" value="{{ $consultaId->piezas }}">
            <p class="text-primary fst-Italic">
              {{ $errors->first('txtcodigo') }}
            </p>
          </div>
